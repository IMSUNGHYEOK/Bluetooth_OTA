...
import com.qualcomm.qti.EMTechOTA.receivers.BREDRDiscoveryReceiver;
import com.qualcomm.qti.EMTechOTA.services.BluetoothService;
import com.qualcomm.qti.EMTechOTA.ui.adapters.DevicesListAdapter;
import com.qualcomm.qti.EMTechOTA.ui.adapters.DevicesListTabsAdapter;
import com.qualcomm.qti.EMTechOTA.ui.fragments.DevicesListFragment;
...
public class DeviceDiscoveryActivity extends BluetoothActivity implements
    ...
    public void onDeviceFound(BluetoothDevice device) {
        if (mDevicesAdapter != null && device != null
                && device.getName() != null && device.getName().length() > 0 && device.getName().toString().contains("EM")) {
    ...
    private void startMainActivity() {
        ...
        intent.putExtra("DeviceName",
                mTabsAdapter.getSelectedDevice().getName().toString());
        intent.putExtra("DeviceAddress",mTabsAdapter.getSelectedDevice().getAddress().toString());
        intent.putExtra("DeviceBond",mTabsAdapter.getSelectedDevice().getBondState());
    ...
    private class LeScanCallback implements BluetoothAdapter.LeScanCallback {
        ...
        public void onLeScan(final BluetoothDevice device, final int rssi, final byte[] scanRecord) {
            if (mDevicesAdapter != null && device != null
                    && device.getName() != null && device.getName().length() > 0 && device.getName().toString().contains("EM")) {
                Log.i("Device Name : " , device.getName().toString());
